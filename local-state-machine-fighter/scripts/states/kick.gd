...
func enter():
	...
	damage.get_node('AnimatedSprite2D').play('red')
	damage.get_node('CollisionShape2DWide').disabled = true
	damage.position.x = -10
	damage.position.y = 10
	damage.rotation = -PI/8
	character_body_2d.add_child(damage)
	damage.velocity.x = 100 * character_body_2d.direction
	damage.velocity.y = -50
